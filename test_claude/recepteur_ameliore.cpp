  ...
  if (currentState != STATE_GAME) {
    if (act == 1) {
      ...
      if (currentState == STATE_MENU_GAMES)
        ...
        currentBrightness = (currentBrightness - 25 < 10) ? 10 : currentBrightness - 25;
        dma_display->setBrightness8(currentBrightness);
      }
    }
    else if (act == 2) {
  ...
  void onResult(BLEAdvertisedDevice advertisedDevice) {
    if (advertisedDevice.getAddress().toString() == targetMac.c_str()) {
      if (advertisedDevice.haveManufacturerData()) {
        ...
        // Format corrigé : [Company_ID_Lo][Company_ID_Hi][code] = 3 bytes minimum
        if (mfgData.length() >= 3) {
...
void drawScore() {
  ...
  String sP1 = inTieBreak ? String(pointsTeam1_raw) :
               (advTeam1 ? "Ad" :
               (pointsTeam1_raw >= 3 && pointsTeam2_raw >= 3 ? "40" : pointStr[pointsTeam1_raw]));
  String sP2 = inTieBreak ? String(pointsTeam2_raw) :
               (advTeam2 ? "Ad" :
               (pointsTeam1_raw >= 3 && pointsTeam2_raw >= 3 ? "40" : pointStr[pointsTeam2_raw]));
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("Démarrage DIGICOURT...");
